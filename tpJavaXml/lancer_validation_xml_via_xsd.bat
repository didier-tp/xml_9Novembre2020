REM .bat à ecrire sous eclipse avec open with ... / text editor
REM .bat à lancer eclipse / windows avec open with ... / system editor
cd /d "%~dp0" 
set JAVA_HOME=C:\Program Files\Java\jdk1.8.0_211

REM produit2.xml et produit2Invalide.xml ne sont pas déjà associés à produit.xsd
REM et necessitent de donner une valeur à XSDPath

REM set XMLPath=src/main/resources/produit.xml
REM set XMLPath=src/main/resources/produit2.xml
REM set XMLPath=src/main/resources/produitInvalide.xml
set XMLPath=src/main/resources/produit2Invalide.xml

set XSDPath=src/main/resources/produit.xsd

REM penser a reconstruire si besoin une nouvelle version de target/tpJavaXml.jar
REM via mvn package ou bien Run as / Maven install d'eclipse .

"%JAVA_HOME%\bin\java" -cp target/tpJavaXml.jar fr.xyz.tp.xml.ValiderXmlViaXsd %XMLPath% %XSDPath%
pause
